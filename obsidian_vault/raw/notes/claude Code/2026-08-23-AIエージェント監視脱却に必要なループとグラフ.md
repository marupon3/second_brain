...
tags:
  ...
  - graph
---
![[Pasted image 20260825171051.png]]
# 要約

このポストは、AIエージェントを常時監視せずに運用するには「ループ」と「グラフ」の両方が必要だと主張する。
ループは単位作業を正しく完了させる仕組み（生成・チェック・修正の反復）で、チェックが失敗可能でなければ単なるスケジューラに過ぎない。
グラフは作業の全体構造（並列実行、依存関係、ゲート、戻り経路）を決め、不要な待ちや誤った順序を排除する。
多くの人はループだけを作り、グラフを欠くため限界に達する。
ノード種別（スプリッター、ワーカー、コード、ゲート）や2種類の戻り経路、ブラスト半径に基づくゲート開閉などを詳述し、人間は最高リスクの1点だけを承認すべきと説く。詳細はagent-layers.vercel.appを参照
...
A loop makes a unit of work correct without you.  
A graph decides which units exist at all.
...
![[Pasted image 20260825171358.png]]
...
A node is a unit of work: one bounded job, one input in, one output out.  
...
The full breakdown of all five layers, in the order they matter, is here: [agent-layers.vercel.app](https://agent-layers.vercel.app)
...
![[Pasted image 20260825171425.png]]
...
![[Pasted image 20260825171447.png]]
...
![[Pasted image 20260825171611.png]]
...
I put all five layers into a full course, twenty lessons with the templates, the configs and the order to build them in: [agent-layers.vercel.app](https://agent-layers.vercel.app)
