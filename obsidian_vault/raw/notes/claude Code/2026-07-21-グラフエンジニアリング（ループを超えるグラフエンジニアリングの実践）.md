...
tags:
  ...
  - graph
---
![[Pasted image 20260823092128.png]]
# 要約

直線的なマルチステップエージェントは非効率で、作業を「ノードとエッジ」のグラフとして設計すべきだと主張。
Claude Codeの動的ワークフローで並列実行・検証・隔離を実装する5ステップを解説し、単一ループの限界（Goodhartの法則など）を超え、独立した作業を同時に広げてスケールさせるグラフエンジニアリングの有効性を示す。
