---
created: 2026-08-19
source: https://x.com/polydao/status/2089992951703171220
tags:
  - graph
  - loop
  - context
  - claude
---
![[Pasted image 20260823182331.png]]

# 要約

ループ工学は抽象論ではなく、300エージェントのスウォームで実際に配線すると意味を持つ。
トリガー（手動・閾値・スケジュール・イベント）と予算ガード、グラフによるノード・エッジ設計（証拠付き関係保存）、コンテキスト工学（固定スキーマ返却とスキルファイル制約）の3層が積み重なる。
Kimi K3のAgent Swarmを中核に、ループが実行タイミング、グラフが保持内容、コンテキストがセッション後の生存を決める。
単独では不十分で、三者を組み合わせることで実用的なエージェントシステムになると主張。
