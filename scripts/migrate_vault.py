...
使い方:
    python scripts/migrate_vault.py <移行元Vaultフォルダ> [<移行元Vaultフォルダ2> ...] [--apply] [--force]
...
from convert_onenote_note import ENCODING, output_filename, parse_onenote_markdown, render_note
...
def migrate_root(
    ...
    """1つの移行元Vaultフォルダを処理する。戻り値は (成功件数, スキップ件数, エラー件数)。"""
    print(f"\n#### 移行元: {root} ####")
    ...
    for top_dir in top_level_dirs:
        ...
        for md_path in md_files:
            ...
            dest_path.write_text(rendered, encoding=ENCODING)

            for img_rel in image_paths:
                src_img = (md_path.parent / img_rel).resolve()
...
def main() -> int:
    ...
    for root in args.roots:
        if not root.is_dir():
            ...
            total_error += 1
            continue
        ok, skip, error = migrate_root(root, args.vault_raw, args.apply, args.force, used_paths)
        total_ok += ok
        total_skip += skip
        total_error += error

    print(f"\n#### 集計: 成功 {total_ok} / スキップ {total_skip} / エラー {total_error} ####")
    if not args.apply:
