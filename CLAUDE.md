...
**`wiki/session-handoff.md`の更新タイミング**: 「セッション終了時」はAIが観測できないため更新のトリガーにしない。代わりに次のタイミングで、未解決事項に変化があれば都度更新する。

- 応答を返す前（そのターンで未解決事項の追加・解決・状況変化があった場合）
- 作業対象のトピックを切り替える前
- 破壊的操作を伴わない通常のコミットを作成した直後

未解決事項に変化がなければ、この確認のためだけに書き込みを発生させない（`wiki/session-handoff.md`自体の運用規律である肥大化防止を優先する）。
