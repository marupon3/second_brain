...
4. サブエージェントにも本SKILL.mdの制約（`raw/`は読み取り専用、変動する事実への日付スタンプ、日本語出力、UTF-8明示、絵文字不使用）を明示的に引き継ぐ。

## 制約

- `obsidian_vault/raw/`配下のファイルは読み取りのみとし、一切編集・削除しない（`CLAUDE.md` 4節 禁止事項）。
- 変動する事実（件数・進捗・設定の有無・対応状況・バージョン等）をページ本文に書く場合は、日付スタンプ`（YYYY-MM-DD時点）`を添えるか、真実の所在へのポインタとして書く（`CLAUDE.md` 6.3節）。原文が「現在〜である」と書いていても、そのまま現在形で転記しない。**原文の日付が分かる場合はその日付を使う**（`raw/`のファイル名の日付・本文中の日付等。処理日ではなく観測日を記録する）。
