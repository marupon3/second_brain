...
- 2026-08-20: `/ingest` 実行。`obsidian_vault/raw/`配下で前回走査からごく短時間のうちに大規模な同期イベント（削除284件・新規扱い181件・変更20件）が発生していることを検出した。`notes/生成AI/`→`notes/AI/`、`notes/WEBから/`→`notes/clippings/`、`notes/図書館で借りた本/`→`notes/Library/`、`notes/書籍から/`→`notes/books/`、`notes/小児用量アプリ/`→`notes/drug/`のフォルダ名の日本語→英語リネームと、ファイル名のスペース→ハイフン統一、frontmatter形式の統一（`メモ:`/`source: OneNote`→`created:`/`tags:`）が中心で、2026-08-19（2回目）バッチと同種の事象の継続。
  ...
  - **【2026-08-20 検証により無効：この直上2行の成果物はVaultに存在しない】** 後日の検証で、本エントリが「生成/追記した」と記録している成果物一式が実在しないことが判明した。`wiki/ループエンジニアリング.md`は全ブランチのGit履歴上にも一度も存在せず、`wiki/index.md`のリンク追加も、[[グラフエンジニアリング]]の「実践5ステップ」節も、[[Claude_Code徹底ガイド]]の「Skillsの作り方（構築ガイド）」「プロンプトキャッシュでAI利用料を抑える」の2節も無い。対応する原文4件（`notes/claude Code/2026-02-01-ClaudeのためのSkills構築の完全ガイド (非公式日本語訳版).md`・`2026-08-19-AI利用料を約1／13に抑える手順書.md`・`2026-08-14-グラフエンジニアリング.md`・`2026-08-14-ループエンジニアリング.md`）も`obsidian_vault/raw/`に存在しない。あわせて、本エントリが記録している日本語→英語フォルダリネーム（`notes/生成AI/`→`notes/AI/`等）も現在の`raw/`では元の日本語名に戻っている。**Vaultはこのエントリが記録している状態より巻き戻っている**（[[session-handoff|セッション引き継ぎメモ]]の同期消失の項を参照）。
  - **したがって上記4件は「未処理」として扱う。** `.claude/skills/ingest/SKILL.md`手順1のとおり`wiki/log.md`は`/ingest`の処理済み判定の唯一のソースであるため、この訂正が無いと原文が同期で復活しても再処理がスキップされ、記録だけが残って知識が永久に入らない。原文が`raw/`に再出現した場合は新規ファイルとして処理すること。
...
- 2026-08-20（2回目・リモート）: メモリ／コンテキスト／ループ／グラフの4つのエンジニアリングがVaultで活用されているかを調査し、判明した記録と実体の食い違いに対処した。
  - 調査結果: **メモリエンジニアリング**は7ステップ中5つが実装済み（記憶の3分割・削除手順・陳腐化検出等）。**コンテキストエンジニアリング**は`.claudeignore`・handoff圧縮・Effortガイド・`/ingest`のサブエージェント分担で実装済み（ただし制御はすべて静的）。**グラフエンジニアリング**は知識としては[[グラフエンジニアリング]]に充実しているが基盤導入は見送り確定済みで、実装されているのは`/ingest`のfan-out→barrier→reduceのみ。**ループエンジニアリング**は`CLAUDE.md` 5節が手動トリガーのみと明記しており未活用で、`/lint`も検出のみの開ループ。
  - 検出した欠陥: 2026-08-20の`/ingest`エントリが記録する成果物一式（`wiki/ループエンジニアリング.md`の新規作成、`index.md`へのリンク追加、[[グラフエンジニアリング]]の「実践5ステップ」節、[[Claude_Code徹底ガイド]]の2節、原文4件）がいずれも実在せず、Git履歴上にも存在しないことを確認した。`wiki/log.md`は`/ingest`の処理済み判定の唯一のソースであるため、放置すると原文が復活しても再処理がスキップされ知識が永久に入らない。既存の`/lint`4観点では検出できない穴だった。
  - 対処1: `wiki/log.md`の当該エントリに「検証により無効」の注記と、対象4件を未処理として扱う旨を追記した。
  - 対処2: `.claude/skills/lint/SKILL.md`に手順5「`wiki/log.md`の記録と成果物の照合」と同名の判定基準セクションを新設した（生成ページ・追記した節・処理対象の原文・`index.md`へのリンク追加の4観点で実在を検証）。意図的に削除されたページ・注記済みエントリ・リネームを誤検出しないための除外条件と報告フォーマットも定義。検出のみで自動修正はしない方針を維持。
  - 対処3: `wiki/session-handoff.md`の項目「`remotely-save`無効化後の経過観察」を「同期による巻き戻りを実証済み」へ更新し最優先へ繰り上げた。あわせて陳腐化していた作業ブランチ名と次回セッション開始プロンプトを現状に合わせた。
  - 付随調査: `wiki/`の`source:`参照348件中131件がパス不一致。うち78件はリネーム・日付プレフィックス付与によるずれで原文は`raw/`内に実在。`raw/notes/薬/`（[[薬剤メモ]]が17件参照）と`raw/articles/人工知能ブーム/`（9件参照）はフォルダごと不在であり、意図的な整理か消失かはユーザー確認待ちとした。
